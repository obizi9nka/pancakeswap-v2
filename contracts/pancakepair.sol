...
import './utils/libraries/UQ112x112.sol';

import './utils/PancakeERC20.sol';
import './utils/libraries/Math.sol';
import './utils/interfaces/IPancakeCallee.sol';
...
contract PancakePair is PancakeERC20 {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IERC200(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        uint balance0 = IERC200(token0).balanceOf(address(this));
        uint balance1 = IERC200(token1).balanceOf(address(this));
    ...
    function skim(address to) external lock {
        address _token0 = token0; // gas savings
        address _token1 = token1; // gas savings
        _safeTransfer(_token0, to, IERC200(_token0).balanceOf(address(this)).sub(reserve0));
        _safeTransfer(_token1, to, IERC200(_token1).balanceOf(address(this)).sub(reserve1));
    }

    // force reserves to match balances
    function sync() external lock {
        _update(IERC200(token0).balanceOf(address(this)), IERC200(token1).balanceOf(address(this)), reserve0, reserve1);
    }
}